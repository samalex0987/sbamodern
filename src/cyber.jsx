...
const CyberResiliencyPage = () => {
  ...
  const currentFeatureIndex = hoveredFeature !== null ? hoveredFeature : activeFeature;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-black">
      ...
      <div className="absolute inset-0 overflow-hidden">
        ...
        <div className="absolute -bottom-40 -left-40 w-80 h-80 bg-red-500 rounded-full mix-blend-multiply filter blur-xl opacity-15 animate-pulse animation-delay-2000"></div>
        <div className="absolute top-40 left-40 w-60 h-60 bg-red-700 rounded-full mix-blend-multiply filter blur-xl opacity-15 animate-pulse animation-delay-4000"></div>
      </div>

      <div className="relative z-10">
        {/* Hero Section */}
        <section className={`px-6 py-20 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          <div className="max-w-6xl mx-auto text-center">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-red-500/10 border border-red-500/30 rounded-full mb-8 backdrop-blur-sm hover:bg-red-500/20 hover:border-red-500/50 transition-all duration-500 hover:scale-105 hover:shadow-lg hover:shadow-red-500/20">
              <Shield className="w-5 h-5 text-red-400 hover:rotate-12 transition-transform duration-300" />
              <span className="text-red-300 font-medium">Our Data Protection Solution</span>
            </div>
            
            <h1 className="text-6xl md:text-7xl font-bold text-white mb-6 leading-tight hover:scale-105 transition-transform duration-700">
              The Foundation of
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <button  onClick={ ()=>{ window.location.href = "/Connect-with-us" }} className="group cursor-pointer px-8 py-4 border-2 border-red-500/50 text-red-300 font-semibold rounded-lg hover:bg-red-500/20 hover:border-red-400 hover:text-white hover:shadow-lg hover:shadow-red-500/20 transition-all duration-300 hover:scale-105">
        ...
        <section className="px-6 py-20">
          <div className="max-w-6xl mx-auto">
            <div className="bg-white/5 backdrop-blur-lg border border-red-500/20 rounded-3xl p-12 hover:bg-red-500/5 hover:border-red-500/40 hover:shadow-2xl hover:shadow-red-500/10 transition-all duration-700 hover:scale-[1.02] group">
              <h2 className="text-4xl font-bold text-white mb-6 text-center group-hover:text-red-100 transition-colors duration-300">Beyond Backup: The Solution Differentiator</h2>
              <p className="text-xl text-gray-300 text-center max-w-4xl mx-auto leading-relaxed group-hover:text-gray-200 transition-colors duration-300">
        ...
        <section className="px-6 py-20">
          <div className="max-w-6xl mx-auto">
            <h2 className="text-4xl md:text-4xl text-3xl font-bold text-white mb-4 text-center">Core Cyber Resiliency Features</h2>
            <p className="text-gray-300 text-center mb-16 text-lg px-4">
            ...
            <div className="hidden lg:grid lg:grid-cols-2 gap-8">
                ...
                {features.map((feature, index) => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <h3 className="text-xl font-semibold text-white group-hover:text-red-100 transition-colors duration-300">{feature.title}</h3>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="bg-white/5 backdrop-blur-lg border border-red-500/20 rounded-xl p-8 h-fit hover:bg-red-500/5 hover:border-red-500/40 hover:shadow-xl hover:shadow-red-500/10 transition-all duration-500 hover:scale-[1.02]">
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <h3 className="text-2xl font-bold text-white">{features[currentFeatureIndex].title}</h3>
                </div>
                <p className="text-gray-300 leading-relaxed text-lg">{features[currentFeatureIndex].description}</p>
                
                {currentFeatureIndex === 2 && (
                  <div className="mt-8 space-y-4">
                    <h4 className="text-lg font-semibold text-white mb-4">Our features include:</h4>
                    {recoveryFeatures.map((recoveryFeature, index) => (
                      <div key={index} className="group flex items-start gap-3 p-4 bg-white/5 rounded-lg hover:bg-red-500/10 hover:scale-105 transition-all duration-300 cursor-pointer">
                        <div className="p-2 bg-red-600 rounded text-white mt-1 group-hover:scale-110 group-hover:rotate-6 transition-all duration-300 shadow-lg shadow-red-600/30">
                          {recoveryFeature.icon}
                        </div>
                        <div>
                          <h5 className="font-semibold text-white mb-2 group-hover:text-red-100 transition-colors duration-300">{recoveryFeature.title}</h5>
                          <p className="text-gray-300 text-sm group-hover:text-gray-200 transition-colors duration-300">{recoveryFeature.description}</p>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            <div className="lg:hidden space-y-6">
              {features.map((feature, index) => (
                <div key={index} className="bg-white/5 backdrop-blur-lg border border-red-500/20 rounded-xl overflow-hidden hover:bg-red-500/5 hover:border-red-500/40 hover:shadow-xl hover:shadow-red-500/10 transition-all duration-500 hover:scale-[1.02]">
                  {/* Feature Header */}
                  <div 
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className="p-3 bg-red-600 rounded-lg text-white shadow-lg shadow-red-600/30 group-hover:scale-110 group-hover:rotate-6 transition-all duration-300">
                        {feature.icon}
                      </div>
                      <div className="flex-1">
                        <h3 className="text-lg font-semibold text-white group-hover:text-red-100 transition-colors duration-300">{feature.title}</h3>
                        ...
                        {index === 2 && activeFeature === index && (
                          <div className="space-y-4">
                            ...
                            {recoveryFeatures.map((recoveryFeature, recoveryIndex) => (
                              <div key={recoveryIndex} className="flex items-start gap-3 p-4 bg-white/5 rounded-lg">
                                <div className="p-2 bg-red-600 rounded text-white mt-1 shadow-lg shadow-red-600/30">
                                  {recoveryFeature.icon}
                                </div>
                                <div>
                                  <h5 className="font-semibold text-white mb-2">{recoveryFeature.title}</h5>
                                  <p className="text-gray-300 text-sm">{recoveryFeature.description}</p>
                                </div>
                              </div>
                            ))}
                          </div>
        ...
        <section className="px-6 py-20">
          <div className="max-w-6xl mx-auto">
            <h2 className="text-4xl font-bold text-white mb-16 text-center">Why Choose Our Solution?</h2>
            
            <div className="grid md:grid-cols-3 gap-8">
              {benefits.map((benefit, index) => (
                <div 
                  key={index} 
                  className="group bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-lg border border-white/10 rounded-xl p-8 hover:shadow-2xl hover:shadow-red-500/20 hover:border-red-500/30 hover:bg-red-500/5 transition-all duration-700 hover:scale-110 hover:-translate-y-3 cursor-pointer"
                >
                  <div className="p-4 bg-gradient-to-r from-red-600 to-red-700 rounded-xl text-white mb-6 w-fit group-hover:rotate-12 group-hover:scale-110 transition-all duration-500 shadow-lg shadow-red-600/30">
                    {benefit.icon}
                  </div>
                  <h3 className="text-xl font-bold text-white mb-4 group-hover:text-red-100 transition-colors duration-300">{benefit.title}</h3>
        ...
        <section className="px-6 py-20">
          <div className="max-w-4xl mx-auto text-center">
            <div className="bg-gradient-to-r from-red-600/10 to-red-700/10 backdrop-blur-lg border border-red-500/30 rounded-3xl p-12 hover:from-red-600/20 hover:to-red-700/20 hover:border-red-500/50 hover:shadow-2xl hover:shadow-red-500/20 transition-all duration-700 hover:scale-[1.02] group">
              <h2 className="text-4xl font-bold text-white mb-6 group-hover:text-red-100 transition-colors duration-300 hover:scale-105 transition-transform">Ready to Transform Your Data Protection?</h2>
              <p className="text-xl text-gray-300 mb-8 group-hover:text-gray-200 transition-colors duration-300">
                Don't wait for the next cyber attack. Secure your business with true cyber resiliency today.
              </p>
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <button onClick={ ()=>{ window.location.href = "/Connect-with-us"} } className="group/btn cursor-pointer px-10 py-4 border-2 border-red-500/50 text-red-300 font-semibold rounded-lg hover:bg-red-500/20 hover:border-red-400 hover:text-white hover:shadow-lg hover:shadow-red-500/20 transition-all duration-300 hover:scale-105 hover:-translate-y-1">
